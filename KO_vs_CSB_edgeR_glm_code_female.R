...
##R code for EdgeR
library(edgeR)
combined_counts <- as.data.frame(combined_counts_female)
...
View(norm_data)
group <- c(rep("Obp56h_KO_F",2),rep("CSB_F",2))
group<-factor(group)
...
KO_vs_CSB <-makeContrasts(Obp56h_KO_F-CSB_F,levels=design)
res <- glmLRT(fit,contrast = KO_vs_CSB)
is.de<-decideTestsDGE(res)
summary(is.de)
plotMD(res,status = is.de,values=c(1,-1),col=c("blue","red"),legend="topright", main="KO vs CSB (females)")
...
KO_vs_CSB_output <- topTags(res,n=7533)
write.csv(KO_vs_CSB_output,file="KO_vs_CSB_female.csv")
